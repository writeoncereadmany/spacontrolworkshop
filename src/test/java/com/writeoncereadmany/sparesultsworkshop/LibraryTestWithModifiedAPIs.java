...
public class LibraryTestWithModifiedAPIs {
    ...
    public void canBorrowABook() {
        given(mapper.newReadObject(anyString())).willReturn(success(enquiry));
        given(auth.newAuthenticate(enquiry)).willReturn(success(enquiry));
        given(borrowings.newBorrow(book)).willReturn(success(book));

        assertThat(
            library.functionalBorrowWithModifiedAPIs("{ \"user\": \"Tom\", \"password\": \"letmein\", \"isbn\": \"123\" }"),
            is("lots of words"));
    }

    @Test
    public void reportsMalformedRequest() {
        given(mapper.newReadObject(anyString())).willReturn(failure("Malformed request"));

        assertThat(
            library.functionalBorrowWithModifiedAPIs("i would like a book plz kthxbai"),
    ...
    public void rejectsUnauthorizedUser() {
        given(mapper.newReadObject(anyString())).willReturn(success(enquiry));
        given(auth.newAuthenticate(enquiry)).willReturn(failure("Unauthorized"));
    ...
    public void cannotLendUnknownBook() {
        ...
        assertThat(
            library.functionalBorrowWithModifiedAPIs("{ \"user\": \"Tom\", \"password\": \"letmein\", \"isbn\": \"456\" }"),
            is("Book not found"));
    }
    ...
    public void cannotLendBookIfAlreadyWithdrawn() {
        given(mapper.newReadObject(anyString())).willReturn(success(enquiry));
        given(auth.newAuthenticate(enquiry)).willReturn(success(enquiry));
        given(borrowings.newBorrow(book)).willReturn(failure("Book already withdrawn"));
